...
export default function EpcOemPartDetailSheet({
  ...
  const pn = part
    ? String(part.partnumber ?? part.Partnumber ?? part.oe_number ?? "").trim()
    : "";
  ...
  useEffect(() => {
    ...
    const rawPn = String(
      part.partnumber ?? part.Partnumber ?? part.oe_number ?? "",
    ).trim();
  ...
  const fitStr =
    fit === 1 || fit === true ? "Sí" : fit === 0 || fit === false ? "No" : "";
  ...
  const imgRaw =
    p.illustration_img_address ??
    p.Illustration_img_address ??
    p.part_img ??
    p.Part_img;
  const diagramUrl = api.imageUrlFromPart(
    partEpc,
    imgRaw != null ? String(imgRaw) : undefined,
  );
  ...
  const onMenuSelect = (item: MenuItem) => {
    ...
    if (key === "t") {
      ...
      void runAction("6", "6", () =>
        api.partsIllustration({ epc, part_number: pn }),
      );
    }
  };
  ...
  const catalogBlock = (
    ...
    >
      ...
      {callout ? (
        <DetailRow label="Callout (diagrama)" value={callout} />
      ) : null}
  ...
  const lightbox =
    diagramLightboxOpen && illustrationUrls.length > 0
      ? createPortal(
          <AnimatePresence>
            <motion.div
                  ...
                  <>
                    ...
                    <div className="mt-4 flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 backdrop-blur-sm">
                      ...
                      <span className="ml-1 text-[11px] text-white/60">
                        {illustrationIdx + 1}/{illustrationUrls.length}
                      </span>
                    </div>
                  </>
                ) : null}

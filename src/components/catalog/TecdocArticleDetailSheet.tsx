  ...
  return (
    <div className={CARD}>
      <button
        ...
        aria-expanded={open}
      >
        <span className="text-[11px] font-semibold uppercase tracking-widest text-neutral-500">{title}</span>
...
export default function TecdocArticleDetailSheet({
  ...
  const [detailWrap, setDetailWrap] = useState<Record<string, unknown> | null>(null);
  ...
  const product = String(merged.articleProductName ?? merged.genericArticleDescription ?? "");
  ...
  const loadRemusa = useCallback(async (code: string) => {
    ...
    try {
      setRemusaDetail((await api.remusaDetail(code)) as Record<string, unknown>);
    } catch {
  ...
  const specs = (merged.allSpecifications as Array<Record<string, unknown>>) ?? [];
  ...
  const onMenuSelect = useCallback((item: MenuItem) => {
    ...
    if (item.id === "t") {
      if (!artNo) return;
      void run("t", () => api.tecdocPartLookup(artNo, fallbackStr || undefined));
      return;
    }
    if (item.id === "o") {
      setCollapseDetailSections(true);
      ...
      >
        ...
        >
          ...
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-3 py-3 sm:px-4 sm:py-4">
            ...
            <SheetSection title="Artículo TecDoc" collapseSignal={collapseDetailSections}>
              ...
              {imgUrl ? (
                <div className="mt-2 border-t border-neutral-200 pt-3">
                  ...
                  >
                    <HiArrowsPointingOut className="h-4 w-4 shrink-0 text-neutral-500" aria-hidden />
                    Ver imagen
                  </button>
                </div>
              ) : null}
            ...
            {specs.length > 0 ? (
              <SheetSection title={`Especificaciones (${specs.length})`} collapseSignal={collapseDetailSections}>
                <div>
                  {specs.map((s, i) => (
                    <DetailRow
                      key={i}
            ...
            {subPanel === "oems" && oemCodes.length > 0 ? (
              <SheetSection title={`OEM equivalentes (${oemCodes.length})`} collapseSignal={false}>
                <ul className="max-h-48 space-y-1.5 overflow-y-auto">
                  {((merged.oemNo ?? merged.oemNumbers) as Array<Record<string, unknown>>)?.map((o, i) => (
                    <li
                      key={i}
                    ...
                    >
                      <p className="text-[13px] font-semibold leading-snug text-neutral-900">
                        {String(o.oemBrand ?? o.mfrName ?? "").trim() || "OEM"}
                      </p>
            ...
            {subPanel === "cars" && cars.length > 0 ? (
              <SheetSection title={`Vehículos compatibles (${cars.length})`} collapseSignal={false}>
                <div className="space-y-3">
                  <p className="text-[11px] text-neutral-500">
                    {cars.length} vehículo{cars.length === 1 ? "" : "s"} compatible{cars.length === 1 ? "" : "s"}
                  </p>
                  <ul className="space-y-1.5">
                    {cars.map((car, j) => {
                      const modelName = String(car.modelName ?? car.carName ?? "—");
                      const mfr = String(car.manufacturerName ?? "").trim();
                      const engine = String(car.typeEngineName ?? car.carName ?? "").trim();
                      const start = String(car.constructionIntervalStart ?? "?").slice(0, 7);
                      const endRaw = String(car.constructionIntervalEnd ?? "").trim();
                      ...
                      return (
                        <li
                          ...
                          className="rounded-xl border border-neutral-200/80 bg-white p-2.5 shadow-sm"
                        >
                          <p className="text-[13px] font-semibold leading-snug text-neutral-900">{modelName}</p>
                    ...
                    <li key={item.id}>
                      <MenuTerminalRow
                        ...
                        disabled={
                          ...
                          (item.id === "e" && !artNo && oemCodes.length === 0) ||
            ...
            {actionPayload != null && !actionKey ? (
              <div className="rounded-2xl border border-neutral-200/90 bg-white p-3 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.08)]">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-[#75141C]">Resultado</p>
